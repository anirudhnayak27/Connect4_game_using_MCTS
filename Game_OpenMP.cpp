...
// #pragma omp declare reduction(max_node : Node* : omp_out = (omp_out == nullptr || omp_in->value() > omp_out->value()) ? omp_in : omp_out)
...
public:
    ...
    std::pair<Node*, ConnectState> select_node() {
        ...
        while (!node->children.empty()) {
            ...
            for (i = 0; i < static_cast<int>(children.size()); ++i) {
                Node* child = children[i];
                double child_value = child->value();
